...
bbbfly.grid._onUpdated=function(){var b=this.ControlsPanel?this.ControlsPanel:this,c=b.ChildControls;this._Rows=[];this._Columns=[];if(c&&0<c.length){var a=b.Elm();ng_BeginMeasureElement(a);b=ng_ClientWidth(a);ng_EndMeasureElement(a);var f=c.length,h=Number.isNumber(this.MinColumnWidth)?Math.floor(b/this.MinColumnWidth):1,k=Math.ceil(f/h);h=Math.ceil(f/k);1>h&&(h=1);f=Math.floor(b/h);Number.isNumber(this.MaxColumnWidth)&&(f=Math.min(f,this.MaxColumnWidth));var d=0,e=0;for(l in c)a=c[l],this._Rows[e]||
(this._Rows[e]=[]),this._Columns[d]||(this._Columns[d]=[]),this._Rows[e].push(a),this._Columns[d].push(a),a._GridRow=e,a._GridColumn=d,e++,e===k&&(e=0,d++);c=0;for(var g in this._Rows){var l=this._Rows[g];k=0;for(var m in l)a=l[m],d=a._GridColumn*f,e=f,a._GridColumn+1===h&&(e=b-d,Number.isNumber(this.MaxColumnWidth)&&(e=Math.min(e,this.MaxColumnWidth))),a.SetBounds({T:c,L:d,W:e})&&a.Update(),d=a.Bounds.H,Number.isNumber(d)||(a=a.Elm(),ng_BeginMeasureElement(a),d=ng_ClientHeight(a),ng_EndMeasureElement(a)),
d>k&&(k=d);c+=k}this.AutoSize&&((g=this.ControlsPanel)&&g.Bounds&&(Number.isNumber(g.Bounds.T)&&(c+=g.Bounds.T),Number.isNumber(g.Bounds.B)&&(c+=g.Bounds.B)),this.SetBounds({H:c})&&this.Update(!1),"function"===typeof this.OnAutoSized&&this.OnAutoSized())}return!0};
